...
Checkboard::Checkboard():len{8}, white_castling{true}, black_castling{true}, castling_done{false}, under_check{false}, cant_castle{false}, checkmate{false}, stalemate{false}, pieces_amount{0}, pawn_to_queen{false}, response{} { // creating default position of checkboard
    for(int i = 0; i < 8; ++i)
    {
...
std::string Checkboard::returnBoard() const { // printing the boards
    std::string output;
    output += "---------------------------------------------------\n";
    for(int i = 0; i < len; ++i){
        for(int j = 0; j < len; ++j){
            output += std::string(" ") + board[i][j] + std::string(" ");
        }
        output += "\n";
    }
    output += "---------------------------------------------------\n";
    if (input_system.get_currentPlayer() == WHITE)
        ...
        output += "Black's turn\n";
    if(under_check)
        output += "The king is under check\n";
    return output;
}
...
void Checkboard::__invalid_move () {
    response = input_system.invalid_move_json();
    std::cout << response.dump() << std::endl;
    // exit(EXIT_FAILURE);
}